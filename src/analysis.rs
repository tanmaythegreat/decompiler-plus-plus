...
/// A cheap read of a binary's symbol table, done before the (slower) full
/// analysis pipeline runs, so a front end can decide whether it's worth
/// asking the user about FLIRT / AI-assisted naming at all.
pub struct BinaryProfile {
    /// No named, sized `Text` symbol exists — function starts will have to
    /// be recovered by scanning (see `scan_function_starts`) rather than
    /// read straight from a symbol table, and every one of them will start
    /// out as `sub_XXXXXX`.
    pub stripped: bool,
    /// No dynamic symbol table at all — libc and friends were linked
    /// directly into `.text` instead of being pulled in from a shared
    /// object at load time, which is exactly the case FLIRT exists for.
    pub static_linked: bool,
}

/// Probe a binary's symbol table without running the full analysis
/// pipeline. Intended to be called right after `open_object`, on the same
/// bytes that will go on to `analyze_bytes`.
pub fn probe_binary(path: &str, bytes: &[u8]) -> Result<BinaryProfile, String> {
    let obj = open_object(path, bytes)?;
    let has_named_funcs = obj
        .symbols()
        .filter(|s| s.kind() == SymbolKind::Text && s.size() > 0)
        .any(|s| s.name().map(|n| !n.is_empty()).unwrap_or(false));
    Ok(BinaryProfile {
        stripped: !has_named_funcs,
        static_linked: obj.dynamic_symbols().next().is_none(),
    })
}
...
pub fn analyze_bytes(path: &str, bytes: &[u8], sig_path: Option<&str>, custom_sigs: Option<&[crate::flirt::CustomSig]>) -> Result<Program, String> {
    ...
    if obj.format() == object::BinaryFormat::Pe {
        ...
        // Use compile-time embedded signatures generated by build.rs from libc.a,
        // libgcc.a, and all CRT .o files — covers every global AND local symbol,
        // including internal helpers like __vfprintf_internal, __divdi3, _start, etc.
        // Called once; deserialized from the embedded blob, no file I/O at runtime.
        let libc_sigs = crate::default_sigs::load_libc_sigs();
